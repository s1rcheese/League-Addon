    ...
    private void doPlace() {
        ...
        BlockIterator.register((int)Math.ceil(this.placeRange.get()), (int)Math.ceil(this.verticalRange.get()), (blockPos, blockState) -> {
            ...
            for (LivingEntity target : this.targets) {
                ...
                if (CaUtils.rayTraceCheck(blockPos, false) == null && ((Boolean)this.oldMode.get()).booleanValue()) {
                    if (this.rayTrace.get() == Type.Place || this.rayTrace.get() == Type.Both) {
                        return;
                    }
                    if (!checkVector.isInRange(this.playerPos.add(eyeHeight), (Double)this.placeWallsRange.get())) {
                        return;
                    }
                }
